...
namespace com.amtec.action
{
    public class CommonFunction
    {
        ...
        {
            int iProcessLayer = -1;
            /*string[] productionStepResultKeys = new string[] { "PROCESS_LAYER" };
            string[] productionStepResultValues = new string[] { };
            int errorCode = imsapi.trGetNextProductionStep(sessionContext, stationNumber, serialNumber, "-1", 1, 1, 0, productionStepResultKeys, out productionStepResultValues);
            LogHelper.Info("Api trGetNextProductionStep: serial number =" + serialNumber + ", result code =" + errorCode);
            if (errorCode == 0)
            {
                if (productionStepResultValues.Length == 0)
                {
                    string[] uploadInfoResultKeys = new string[] { "PROCESS_LAYER" };
                    string[] uploadInfoResultValues = new string[] { };
                    errorCode = imsapi.trGetSerialNumberUploadInfo(sessionContext, stationNumber, -1, serialNumber, "-1", 0, uploadInfoResultKeys, out uploadInfoResultValues);
                    LogHelper.Info("Api trGetSerialNumberUploadInfo: serial number =" + serialNumber + ", result code =" + errorCode);
                    if (errorCode == 0)
                    {
                        iProcessLayer = Convert.ToInt32(uploadInfoResultValues[0]);
                    }
                }
                else
                {
                    iProcessLayer = Convert.ToInt32(productionStepResultValues[0]);
                }
            }*/
        ...
        public string GetSiteExtNoByStationNo(string stationNumber)
        {
            int error = 0;
            string siteExtNo = stationNumber;
            KeyValue[] machineAssetStructureFilter = new KeyValue[] { new KeyValue("STATION_NUMBER", stationNumber) };
            string[] machineAssetStructureResultKeys = new string[] { "SITE_NUMBER_EXT" };
            string[] machineAssetStructureValues = new string[] { };
            error = imsapi.mdataGetMachineAssetStructure(sessionContext, init.configHandler.StationNumber, machineAssetStructureFilter, machineAssetStructureResultKeys, out machineAssetStructureValues);
            LogHelper.Info("Api mdataGetMachineAssetStructure station no =" + stationNumber + " , result code =" + error);
            if (error == 0)
                siteExtNo = machineAssetStructureValues[0];
            LogHelper.Info("Site ext no =" + siteExtNo);
            return siteExtNo;
